...
To start the program, run ```python audio_converter.py```
...
- Press the "**Convert Audio**" button and a warning will show once it has completed/failed.
...
- ```PyInstaller --noconfirm --onedir --windowed --name "Audio Converter" --add-data "convert.py;." --add-data "icons/moon.png;icons" --add-data "icons/sun.png;icons" --add-binary "ffmpeg.exe;." --collect-all tkinterdnd2 audio_converter.py``` (saves the python app as a portable executable).
